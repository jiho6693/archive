...
function generateFilename(url) {
    let safeName = encodeURIComponent(url.replace(/[^a-zA-Z0-9]/g, '_'));
    ...
    try {
        ...
        for (const rowData of jsonData.table.rows) {
            if (rowData.c[2] != null) {
                ...
                if (isURL(url)) {
                    const pageName = generateFilename(url); // 수정된 파일 이름 생성 함수 사용
                    const screenshotPath = path.join(__dirname, 'images', pageName);
                    ...
                    htmlImages.push(`
                        <a href="${url}" target="_blank">
                            <img src="./images/${pageName}" alt="${firstColumnValue},${secondColumnValue}" style="max-width: 100%;">
                        </a>
                    `);
                }
            }
